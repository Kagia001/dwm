...
void monocle(Monitor *m) {
  ...
  for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
    resize(c, m->wx - c->bw, m->wy - singularborder_baradjustment(c), m->ww,
           m->wh - c->bw * m->showbar, False);
}
...
int singularborder_baradjustment(Client *c) {
...
void tile(Monitor *m) {
  ...
  for (i = my = ty = 0, c = nexttiled(m->clients); c;
    ...
    if (i < m->nmaster) {
      ...
      h = (m->wh - ty) / (n - i);
      resize(c, m->wx + mw - c->bw,
             m->wy + ty - singularborder_baradjustment(c), m->ww - mw,
             h - c->bw * m->showbar, False);
      ty += HEIGHT(c) - c->bw;
    }
}
